...
| **W5: 통제·운영** | ⬅️ 다음 작업 | 체크섬/diff 게이트 강화, 리뷰어 모델 분리, 비용/시간 telemetry, `agentctl list/cancel/cleanup/daemon`, 웹 대시보드/원격 brief 투입 | "실제로 맡길 수 있음" 기준: 상태 확인, 중단, 정리, 실패 원인 추적, deterministic gate audit 가능 |
