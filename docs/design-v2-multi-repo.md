...
| **W5: 통제·운영** | ✅ 완료 | 체크섬/diff 게이트 강화, cancel/cleanup/resume, 리뷰어 모델 분리, 비용/시간 telemetry, Hono API, `agentctl list/cancel/cleanup/daemon`, 최소 웹 대시보드, single-container Docker skeleton, Docker HTTP/API/static smoke | PR #25 기준 develop 반영. Docker image build/compose health/API/dashboard smoke 완료. 실제 Claude/Codex worker 2-job smoke는 인증/CLI/비용 준비 후 별도 실행 |
| **W6: 운영 확장** | ⬜ 예정 | live worker 2-job smoke, 3~5 job soak/nightly run, dashboard analytics, notification, GitHub Issue/Jira write-back, provider backoff, public auth, Docker worker hardening, split containers/TUI 검토 | W5 MVP 이후 실제 사용 피드백으로 범위를 확정 |
