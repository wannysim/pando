...
주의: SPEC/PLAN이 Atlassian·Figma MCP를 쓰므로 헤드리스 호출 시 `--mcp-config`로 MCP 서버를 명시 주입해야 한다 (데몬 환경엔 네 대화형 세션의 MCP 설정이 없다). Atlassian Remote MCP는 OAuth라 데몬용 토큰 갱신 처리 필요 — 막히면 SPEC 수집만 REST API 직접 호출로 대체하는 게 단순할 수 있음.
...
| 단계 | 내용 | 비고 |
|---|---|---|
| **W1: 헤드리스 검증** | 데몬 없이 셸 스크립트로: worktree 생성(§3.2 데몬 방식) → `claude -p "/implement-jira AP-X --batch"` → PLAN.md 확인 → `codex exec`로 IMPL 1회 → 게이트 수동 확인 | dispatch-batch가 이미 검증한 구간 + IMPL만 신규. **MCP 헤드리스 주입이 최대 리스크 — 여기서 먼저 뚫어라** |
| **W2: 파이프라인 데몬** | TS 데몬 + SQLite 상태머신 + 단계별 게이트 + retry budget. 단일 레포(web), 단일 in-flight | `repos.yaml` 프로파일 도입 |
| **W3: brief 경로** | intake CLI/템플릿 + personal-site 프로파일 → 개인 프로젝트 E2E | 이 시점에 n×1 → n×n 준비 완료 |
| **W4: n×n 병렬** | BullMQ + 3계층 동시성 + 포트/캐시 격리 + TUI 대시보드 | web 3 + personal-site 2 동시 |
| **W5: 통제·운영** | 체크섬/diff 게이트 강화, 리뷰어 모델 분리, 비용 추적, Tailscale 원격 brief 투입 | dispatch-batch 설계노트의 "3→5티켓 검증" 방식으로 점증 |
