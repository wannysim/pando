...
| **W2-B: 파이프라인 결합** | ⬅️ 현재 다음 작업 | stage config loader, artifact/exit-code gates, fake engine 기반 runner skeleton, 단일 in-flight loop | 아직 SQLite persistence와 agentctl은 붙지 않음 |
| **W2-C: 데몬 최소 루프** | ⬜ 예정 | SQLite jobs/events, retry budget persistence, daemon entrypoint, 최소 `agentctl submit/show/retry` | 단일 레포(web), 단일 in-flight |
