...
- 스케줄러/세마포어/병렬 daemon loop는 fake engine + deterministic gates로 먼저 고정한다.
- 실제 Claude/Codex CLI 실행은 W4 본 구현의 필수 acceptance가 아니라, 별도 smoke로 2개 job만 돌린다.
...
- GitHub Issue intake/write-back, 웹 대시보드/API, Stacked PR 자동화, provider별 정교한 backoff는 W4 범위에서 제외한다.
...
| **W2-C: 데몬 최소 루프** | ✅ 완료(기본형) | SQLite jobs/events/repos, retry budget persistence, 단일 tick daemon loop, 최소 `agentctl submit/show/retry` handler | 장시간 상주 daemon/API는 W4/W5에서 확장 |
...
| **W4: n×n 병렬** | ⬜ 예정 | 인프로세스 3계층 동시성 + 포트/캐시 격리 + 병렬 daemon loop | ADR-001에 따라 BullMQ는 도입하지 않음. SQLite + semaphore 유지 |
| **W5: 통제·운영** | ⬜ 예정 | 체크섬/diff 게이트 강화, 리뷰어 모델 분리, 비용 추적, 웹 대시보드/원격 brief 투입 | dispatch-batch 설계노트의 "3→5티켓 검증" 방식으로 점증 |
