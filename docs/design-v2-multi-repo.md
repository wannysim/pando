...
| **W5: 통제·운영** | ⬅️ 다음 작업 | 체크섬/diff 게이트 강화, cancel/cleanup/resume, 리뷰어 모델 분리, 비용/시간 telemetry, Hono API, `agentctl list/cancel/cleanup/daemon`, 최소 웹 대시보드 | 기준 문서: `docs/w5-operational-readiness.md`. "실제로 맡길 수 있음" 기준: 상태 확인, 중단, 정리, 실패 원인 추적, deterministic gate audit 가능 |
| **W6: 운영 확장** | ⬜ 예정 | dashboard analytics, notification, GitHub Issue/Jira write-back, provider backoff, public auth, split containers/TUI 검토 | W5 MVP 이후 실제 사용 피드백으로 범위를 확정 |
