...
`list`, `cancel`, `cleanup`, `daemon` CLI는 W5 운영 UX에서 추가한다. worktree 경로 규약은 이미 `dispatch --list`/`--cleanup`과 호환되도록 유지한다. W5에서 실제로 여러 job을 맡기려면 최소한 현재 queue/in-flight/terminal 상태 조회, 취소, stale worktree 정리가 필요하다.
