...
> 문서는 W1 실측 기록을 보존하기 위한 참고 자료다. 지속되는 결정은
> `docs/adr/`에만 남긴다.
...
- ✅ 샌드박스 격리: `--ephemeral --cd <worktree> --config 'approval_policy="never"' --sandbox workspace-write`가 headless worker 실행을 유지하면서 worktree 밖(origin web `<sha>`/pando) 무간섭. 정확히 대상 2개 파일만 수정.
