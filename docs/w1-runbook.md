...
확인할 것: `~/.worktrees/web/feat-AP-XXXX` 생성, pnpm install 완료, **원본 ~/Github/web의 체크아웃/working tree가 그대로인지** (네가 작업 중이어도 간섭 없어야 함).
...
- ⚠️ **결함 발견·수정**: `01-worktree.sh`가 `pnpm install` 하드코딩인데 web은 **yarn@1.22.19 모노레포**. setup 훅을 lockfile 감지(yarn/pnpm/npm)로 수정함. → `config/repos.yaml`의 web `setup: "pnpm install"`도 `yarn install`로 정정 필요(미적용).
...
`03-impl-codex.sh`로 codex가 PLAN.md의 PR 1(pts·a-components jest config 표준화)을 구현. (⚠️ 03 스크립트도 완료기준이 `pnpm`이라 yarn 모노레포와 불일치 → 수정함. `repos.yaml`의 `gates`도 전부 `pnpm`으로 남아 정정 필요 — setup만 고쳐짐)
