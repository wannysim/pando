...
See [CLAUDE.md](./CLAUDE.md) for the shared rules for all agents (enforced TDD, `bun run verify`, layering boundaries, gate determinism, YAGNI). Note: CLAUDE.md and docs/ are written in Korean.

Critical workflow rules:

- Use the pinned package manager from `package.json` (`packageManager`) and run `bun run verify` before every commit.
