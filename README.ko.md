...
- Node.js `>=22.13.0`.
- `package.json`에 고정된 package manager인 `bun@1.3.5`.
- `git`.
...
같은 local pando server에서 dashboard까지 서빙하려면 dashboard asset을 한 번
build합니다.
...
- Dashboard URL, 일반적으로 `http://127.0.0.1:3210/dashboard`. Source
  checkout에서는 `dashboard/dist` directory가 있으면 이 asset을 사용합니다.
...
source checkout에서 integrated dashboard를 쓰는 경로는 아래와 같습니다.
...
`bun run pando start`가 출력한 dashboard URL을 여세요.

Dashboard frontend를 개발 중이라면 local API를 바라보는 Vite dev server를
실행합니다.
...
`pandoctl`은 operator CLI입니다. checkout에서는 `bun run pandoctl`로 실행합니다.
...
- `bun run pando start`를 실행 중인 terminal에서 `Ctrl-C`로 daemon을 종료합니다.
...
- Checkout의 `pando start` 경로는 daemon/API를 시작하고, build된
  `dashboard/dist`가 있으면 이를 서빙합니다. Asset이 없으면 위에서 설명한
  Vite dev server를 사용하세요.
