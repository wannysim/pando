...
2. **커밋 전 `bun run verify`**: coverage(전체 85%+, core/gates/scheduler 95%+) + oxlint + tsc 전부 통과. "된 것 같다"는 증거가 아니다.
3. **변경 사이징**: 커밋당 ~100라인 내외, atomic. 하나의 커밋은 하나의 이유.
4. **계층 경계**: `src/core`, `src/pipeline`, `src/scheduler`는 순수 계층 — I/O 임포트 금지(oxlint가 강제). 부수효과는 port 인터페이스 뒤의 adapter(`engines/`, `worktree/`, `reporters/`, `db/`, `intake/`)에만.
...
`docs/README.md`를 먼저 읽을 것 — 현재 작업 큐와 필요한 문서만 고르는 라우터다. `docs/handoff.md`, `docs/practical-adoption-roadmap.md`, `docs/next-session-prompt.md`는 작업 시작점이 아니라 과거 기록이다.
